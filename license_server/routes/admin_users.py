...
#     def get(self, admin_id: int, default: Optional[str] = None) -> Optional[str]:
#         return self._data.get("by_id", {}).get(str(admin_id), default)

#     def is_admin(self, admin_id: int) -> bool:
#         return str(admin_id) in self._data.get("by_id", {})
...
#     for r in rows:
#         role = role_store.get(r.id)   # ถ้าไม่มีจะเป็น None
#         if role is None:
#             # แปลว่าเป็น customer ธรรมดา ยังไม่ใช่ admin → ไม่เอามาแสดง
#             continue
...
#     role = role_store.get(user.id)
#     if role is None:
#         # เป็น customer แต่ยังไม่ถูกกำหนดให้เป็น admin
#         raise HTTPException(status_code=404, detail="Admin not found")

#     return AdminUserOut(id=user.id, name=user.name, email=user.email, role=role)
...
#     current_role = role_store.get(admin_id)
...
#     # ถ้าไม่ใช่ admin ก็ไม่ให้ลบผ่าน endpoint นี้
#     if not role_store.is_admin(admin_id):
#         raise HTTPException(status_code=404, detail="Admin not found")
...
from license_server.models.customer import Customer as AdminUser
...
class RoleStore:
    ...
    def all_ids(self) -> List[int]:
        return [int(k) for k in self._data.get("by_id", {}).keys()]
...
def enrich_with_role(rows: List[AdminUser]) -> List[AdminUserOut]:
    ...
    for r in rows:
        ...
        if role is None:
            # ไม่ใช่ admin (ไม่มีใน role_store) → ข้าม
            continue
        out.append(AdminUserOut.model_validate({
            "id": r.id,
            "name": r.name,
...
def list_admin_users(
    ...
    stmt = select(AdminUser).where(AdminUser.id.in_(admin_ids))

    if search:
        s = f"%{search.lower()}%"
        stmt = stmt.where(
            or_(
                func.lower(AdminUser.name).like(s),
                func.lower(AdminUser.email).like(s),
            )
        )

    # ดึงข้อมูลจาก DB แล้ว enrich role
    rows = db.execute(stmt.order_by(AdminUser.id.desc())).scalars().all()
    enriched = enrich_with_role(rows)
...
def create_admin(payload: AdminUserCreate, db: Session = Depends(get_db)):
    # validate role ก่อน
    role = payload.role or AdminRole.Viewer
    if role not in ROLE_VALUES:
        raise HTTPException(status_code=400, detail="Invalid role")
...
def update_admin(admin_id: int, payload: AdminUserUpdate, db: Session = Depends(get_db)):
    ...
    # ต้องเป็น admin อยู่ก่อนเท่านั้น
    current_role = role_store.get(admin_id)
    if current_role is None:
        raise HTTPException(status_code=404, detail="Admin not found")
