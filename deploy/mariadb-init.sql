...
CREATE TABLE client_credentials (
  ...
  username VARCHAR(100) NOT NULL,
  password_hash VARCHAR(255) NOT NULL,
  is_temp_password TINYINT(1) NOT NULL DEFAULT 0,   -- ★ ใช้กับรหัสชั่วคราว
  temp_password_expires_at DATETIME NULL,           -- ★ เวลาหมดอายุ
  created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_client_credentials_client FOREIGN KEY (client_id) REFERENCES clients(id) ON DELETE CASCADE,
  UNIQUE KEY uq_client_username (username)
) ENGINE=InnoDB;
...
-- SEED: welcome template (skip ถ้ามีแล้ว)
INSERT INTO email_templates (id, slug, name, subject, body, is_html, status)
SELECT UUID(),
       'welcome',
       'Welcome (Default)',
       'Your {{meta.app_name}} Account & License',
