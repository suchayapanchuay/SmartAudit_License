...
def get_activity_logs(
    ...
    if user:
        q = q.filter(ActivityLog.user.contains(user))

    if action:
        q = q.filter(ActivityLog.action.contains(action))

    if since:
        q = q.filter(ActivityLog.created_at >= datetime.fromisoformat(since))

    logs = q.order_by(ActivityLog.created_at.desc()).limit(100).all()
